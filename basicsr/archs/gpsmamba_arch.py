...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=64, inner_rank=128, mlp_ratio=2.):
        ...
        self.freq_conv = nn.Conv2d(dim, dim//2, 3, 1, 1)
        self.freq_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(dim//2, dim//2, 1),
            nn.Sigmoid()
        )
        # Projection to match d_state dimension
        self.freq_proj = nn.Linear(dim//2, d_state)
    ...
    def forward(self, x, x_size, token):
        ...
        x_freq_feat = self.freq_conv(x_freq_mag)
        freq_gate = self.freq_gate(x_freq_feat)
        x_freq_feat = x_freq_feat * freq_gate
        ...
        spatial_prompt = torch.matmul(cls_policy, full_embedding).view(B, n, self.d_state)
        
        # Cross-domain attention fusion
        q = spatial_prompt  # [B, HW, d_state]
        k = x_freq_feat.flatten(2).permute(0, 2, 1)  # [B, HW, C//2]
        # Project k to match d_state dimension
        k = self.freq_proj(k) if hasattr(self, 'freq_proj') else k[:,:,:self.d_state]
        v = k
        attn = torch.softmax((q @ k.transpose(-2, -1)) / math.sqrt(self.d_state), dim=-1)
        prompt = spatial_prompt + attn @ v
